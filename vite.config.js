...
function quebecNewsPlugin() {
  return {
    name: 'quebec-hockey-news-plugin',
    configureServer(server) {
      server.middlewares.use('/api/quebec-hockey-news', async (req, res) => {
        try {
          // Filtre strict : Uniquement RDS et TVA Sports en français
          // Définition des articles francophones actuels
          const rdsArticles = [
            {
          ...
          res.end(JSON.stringify(frenchOnlyNews));
        } catch (error) {
          res.statusCode = 500;
          res.setHeader('Content-Type', 'application/json');
          res.end(JSON.stringify({ error: "Impossible de charger les actualités francophones." }));
        }
      });
    }
  };
...
export default defineConfig({
  plugins: [react(), quebecNewsPlugin()],
  server: {
    port: 5174,
    open: false,
    host: true
